...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting rich data extraction for product:', productId, 'URL:', websiteUrl);
    ...
    if (updateError) {
      console.error('Error updating product with rich data:', updateError);
      return NextResponse.json({ 
        error: 'Failed to update product with rich data' 
      }, { status: 500 });
    }

    return NextResponse.json({ 
      message: 'Rich data extracted and product updated successfully',
