...
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      console.error('Error updating service with logo:', updateError);
      return NextResponse.json({ 
        error: 'Failed to update service with logo' 
      }, { status: 500 });
    }

    return NextResponse.json({ 
      message: 'Logo extracted and service updated successfully',
